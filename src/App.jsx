      ...
      <main style={{maxWidth:"1200px",margin:"0 auto",padding:"0 20px 80px",opacity:animIn?1:0,transform:animIn?"translateY(0)":"translateY(6px)",transition:"all 0.2s ease"}}>
        ...
        {tab==="accueil"&&(
          <div>
            ...
            {/* Pays */}
            <h2 style={{fontFamily:FT,fontSize:"17px",fontWeight:700,color:C.dark,margin:"0 0 12px"}}>Pays couverts</h2>
            {["Ouest","Centrale"].map(region=>(
              <div key={region} style={{marginBottom:"12px"}}>
                <div style={{fontSize:"10px",fontWeight:700,color:C.sub,marginBottom:"7px",textTransform:"uppercase",letterSpacing:"0.09em",fontFamily:F}}>
                  {region==="Ouest"?"Afrique de l'Ouest":"Afrique Centrale"}
                </div>
                <div style={{display:"flex",gap:"5px",flexWrap:"wrap"}}>
                  {COUNTRIES.filter(c=>c.region===region).map(c=>(
                    <button key={c.name} onClick={()=>{setFilterCountry(c.name);switchTab("biens");}} style={{background:C.white,border:`1px solid ${C.sand}`,borderRadius:"5px",padding:"5px 10px",fontSize:"11px",fontWeight:600,cursor:"pointer",color:C.dark,fontFamily:F,transition:"all 0.15s"}}
                      onMouseEnter={e=>{e.currentTarget.style.borderColor=C.terra;e.currentTarget.style.color=C.terra;}}
                      onMouseLeave={e=>{e.currentTarget.style.borderColor=C.sand;e.currentTarget.style.color=C.dark;}}>
                      {c.flag} {c.name}
                    </button>
                  ))}
                </div>
